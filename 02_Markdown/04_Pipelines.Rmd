...
```{r bagging}
single_path_xgb = po("subsample") %>>% po("encode") %>>% po("scale") %>>% lrn("classif.xgboost")
graph_bag_xgb = greplicate(single_path_xgb, n = 5) %>>%
    po("classifavg")

graph_bag_xgb

graph_bag_xgb$plot(html=TRUE)
```
...
lrn_graph_bag_xgb
graph_bag_xgb_pred = lrn_graph_bag_xgb$predict_newdata(tsk_attr, attr_newdata)
graph_bag_xgb_pred$score(c(msr_acc, msr_auc))
```

```{r}
lrn_graph_bag_xgb$param_set
